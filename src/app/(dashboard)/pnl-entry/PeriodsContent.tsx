...
import {
  Trash2, Building2, Search,
  ChevronRight, BookOpen, FileText, AlertTriangle, CalendarDays,
} from "lucide-react";
...
export default function PeriodsContent() {
  ...
  async function load() {
    ...
    const raw: Dataset[] = Array.isArray(body?.datasets) ? body.datasets : [];
    setDatasets(
      raw.sort((a, b) =>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 pb-2 border-b">
          ...
          <span className="text-xs text-muted-foreground">— {selected.companyName} · {selected.currency}</span>
          <button
            onClick={() => setConfirm(selected)}
            className="mr-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-red-500 border border-red-200 hover:bg-red-50 dark:border-red-900 dark:hover:bg-red-950/30 transition-colors"
          >
            <Trash2 size={13} />
            حذف الفترة كاملاً
          </button>
                      ...
                      <table className="w-full text-sm">
                        <tbody>
                          {sec.items.map((item, i) => (
                            <tr key={item.key} className={cn("border-b last:border-0", i % 2 === 0 ? "" : "bg-muted/20")}>
        ...
        <div className="space-y-2">
          ...
          {loadingJE ? (
            <div className="bg-card rounded-xl border p-6 text-center text-muted-foreground text-sm">جارٍ التحميل...</div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {journalEntries.map((je, i) => (
                      <tr key={je.id} className={cn("border-b last:border-0", i % 2 === 0 ? "" : "bg-muted/20")}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="bg-card rounded-2xl border shadow-lg w-full max-w-sm p-6 space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <span className="text-orange-500 text-xs mt-1 block">سيتم حذف جميع البنود ضمن هذا التصنيف فقط.</span>
              </p>
